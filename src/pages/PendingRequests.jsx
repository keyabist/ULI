...
const PendingRequests = () => {
    ...
    const fetchPendingLoans = async () => {
      try {
        ...
        for (let i = 1; i < loanCount; i++) {
          ...
          // Skip if not relevant or empty
          if (!loan.borrower || loan.lender === ethers.ZeroAddress) continue;
  ...
  const handleApprove = async (loan) => {
    try {
      ...
      const installmentAmount = ethers.formatUnits(loanData.amount, 18); // ✅ Clean parsing
  ...
  const handleReject = async (loan) => {
    try {
      ...
      const tx = await contract.rejectLoan(BigInt(loan.loanId)); // ✅ Use BigInt
      await tx.wait();

      console.log("Loan rejected:", loan.loanId);
      setPendingLoans((prev) => prev.filter((l) => l.loanId !== loan.loanId));
    } catch (err) {
